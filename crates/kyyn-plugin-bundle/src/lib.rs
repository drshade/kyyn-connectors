//! The JSON-bundle evidence contract shared by component guests.
//!
//! Kyyn locates a record recursively by provider `id` and hashes that
//! record's compact JSON. Keeping the producer-side hash in one small crate
//! lets native tests prove flat and nested bundles round-trip through the
//! same convention before the guest is componentized.
...
/// Normalize a provider-specific identity field into the lowercase `id`
/// locator key Kyyn searches recursively inside JSON bundles.
