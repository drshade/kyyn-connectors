...
mod guest {
    ...
    use graph_evidence_views::{EvidenceReadRequest, EvidenceReadResponse, MailParameters};
    struct Tool;
    ...
    impl Guest for Tool {
        fn invoke(parameters: String) -> Result<Output, Diagnostic> {
            let p: MailParameters = graph_evidence_views::decode(&parameters)
            ...
            let view =
                graph_evidence_views::mail_text(&evidence.content, p.cursor.unwrap_or(0), max)
                    .map_err(|e| fail("unsupported", e))?;
            let local_id = format!("{}:{}", view.message_id, view.byte_start);
            Ok(Output {
                body: Body::Text(
