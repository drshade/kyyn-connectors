...
mod guest {
    ...
    fn validate(config: &Config) -> Result<(), String> {
        ...
        if super::CONNECTOR_KIND == "sharepoint" {
        ...
        let items = emails
            ...
            .map(|email| {
                Ok(Item {
                    ...
                    kind: "email".into(),
                    version: None,
                    content_hash: kyyn_source_bundle::canonical_record_sha256(email)
                        .map_err(|error| error.to_string())?,
    ...
    impl Guest for GraphComponent {
        fn describe() -> ConnectorDescribe {
            ConnectorDescribe {
                name: super::CONNECTOR_NAME.into(),
                link_namespace: if super::CONNECTOR_KIND == "sharepoint" {
                    "sharepoint"
                } else {
                    "graph"
                }
                .into(),
                fetch_style: if super::CONNECTOR_KIND == "sharepoint" {
                    FetchStyle::Snapshot
                } else {
                    FetchStyle::Windowed
                },
        ...
        fn fetch(request: FetchRequest) -> Result<FetchResult, String> {
            ...
            let RunSpec::Window(window) = request.spec else {
                return Err(format!("{} is a windowed source", super::CONNECTOR_NAME));
            };
            match super::CONNECTOR_KIND {
